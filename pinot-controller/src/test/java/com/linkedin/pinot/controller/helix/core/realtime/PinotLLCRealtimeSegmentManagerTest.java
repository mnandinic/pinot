...
public class PinotLLCRealtimeSegmentManagerTest {
  ...
  private void verifyRepairs(TableConfig tableConfig, IdealState idealState,
    ...
    Map<Integer, List<LLCRealtimeSegmentZKMetadata>> latestMetadata =
    ...
    for (int p = 0; p < expectedPartitionAssignment.getNumPartitions(); p++) {
      List<LLCRealtimeSegmentZKMetadata> llcRealtimeSegmentZKMetadata = latestMetadata.get(p);
      ...
      Assert.assertNotNull(llcRealtimeSegmentZKMetadata.get(0));
      Assert.assertNotNull(latestLlcSegment);
      Assert.assertEquals(latestLlcSegment.getSegmentName(), llcRealtimeSegmentZKMetadata.get(0).getSegmentName());
  ...
  public void testCommittingSegmentIfDisconnected() throws InvalidConfigException {
    ...
    TableConfig tableConfig =
        makeTableConfig(tableName, nReplicas, KAFKA_TEST_OFFSET, DUMMY_HOST, DEFAULT_SERVER_TENANT);
  ...
  public void testCommittingSegment() throws InvalidConfigException {
    ...
    SegmentCompletionProtocol.Request.Params reqParams = new SegmentCompletionProtocol.Request.Params();
    TableConfig tableConfig =
        makeTableConfig(rtTableName, nReplicas, KAFKA_TEST_OFFSET, DUMMY_HOST, DEFAULT_SERVER_TENANT);
